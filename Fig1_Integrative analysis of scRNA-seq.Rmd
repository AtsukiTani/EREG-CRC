...
saveRDS(obj_CAC, "~/Primarytumor/object_CAC.Rds")
```
...
saveRDS(obj_merge,"~/Primarytumor/object_KUL3_KUL5_SG1_SG2_SMC.Rds")
```
...
# Filter KUL etc.
sc_1 <- subset(obj_KUL3_KUL5_SG1_SG2_SMC, subset = Sidedness == "L" &  BRAF == "wt" & `MSS/MSI` == "MSS")
sc_2 <- subset(obj_KUL3_KUL5_SG1_SG2_SMC, subset = Sidedness == "R" & `MSS/MSI` == "MSS" & BRAF == "wt")
obj_KUL3_KUL5_SG1_SG2_SMC <- merge(sc_1, y = sc_2)
...
# Quality Control has done in original article
# The input datasets have already been quality-controlled (e.g., mitochondrial filtering, doublet removal)
# as described in their respective original publications. 
# This script starts with the pre-processed data.
```
