...
seu_S2_R1 <- SCTransform(seu_S2_R1, assay = "Spatial", verbose = FALSE)
seu_S2_R2 <- SCTransform(seu_S2_R2, assay = "Spatial", verbose = FALSE)
seu_S4_R1 <- SCTransform(seu_S4_R1, assay = "Spatial", verbose = FALSE)
seu_S4_R2 <- SCTransform(seu_S4_R2, assay = "Spatial", verbose = FALSE)
```
...
p3 <- SpatialFeaturePlot(seu_S2_R1, features = "Myeloid cell1") +
  scale_fill_gradientn(colours = palette_1)
p <- p1 + p2 + p3 + plot_layout(ncol = 3) #SFig5B-1
...
p6 <- SpatialFeaturePlot(seu_S4_R2, features = "Myeloid cell1")+
  scale_fill_gradientn(colours = palette_1)
p <- p4 + p5 + p6 + plot_layout(ncol = 3) #SFig5B-2
...
p8 <- SpatialFeaturePlot(seu_S4_R2, features = "EREG")+
  scale_fill_gradientn(
    colours = palette_2,
    limits = c(0,3))
p <- p7 + p8 + plot_layout(ncol = 2) #Fig6B
...
stroma_annotations <- c("stroma_fibroblastic_IC high", "stroma_fibroblastic_IC med", 
...
seu_S2_R1$spot_name <- case_when(
  seu_S2_R1@meta.data[["Pathologist_Annotation"]] %in% stroma_annotations ~ "Stromal and Immune Spots",
  seu_S2_R1@meta.data[["Pathologist_Annotation"]] %in% tumor_annotations  ~ "Tumor Spot",
  TRUE                                                              ~ "Others" 
)
...
seu_S4_R2$spot_name <- case_when(
  seu_S4_R2@meta.data[["Pathologist_Annotation"]] %in% stroma_annotations ~ "Stromal and Immune Spots",
  seu_S4_R2@meta.data[["Pathologist_Annotation"]] %in% tumor_annotations  ~ "Tumor Spot",
  TRUE                                                              ~ "Others" 
)
...
lfc_S2 <- log2(heatmap_matrix[, "Near_S2_Rep1"] / heatmap_matrix[, "Far_S2_Rep1"])
lfc_S4 <- log2(heatmap_matrix[, "Near_S4_Rep2"] / heatmap_matrix[, "Far_S4_Rep2"])

lfc_matrix <- cbind(S2_Rep1 = lfc_S2, S4_Rep2 = lfc_S4)

max_val <- max(abs(lfc_matrix))
...
p <- pheatmap(
  ...
  main = "Log2 Fold Change (Near vs. Far)",
  number_format = "%.2f",
  breaks = color_breaks 
) #Fig6D
```
